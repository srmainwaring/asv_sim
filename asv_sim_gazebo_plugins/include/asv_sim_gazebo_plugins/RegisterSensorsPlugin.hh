...
namespace asv
{
  ...
  ///
  /// Note:
  /// To display custom message types in 'Gazebo: Text View' the parameter `extra_gazebo_args` 
  /// must be passed to the gazebo client. The default `empty_world.launch` file  
  /// in `gazebo_ros_pkgs/gazebo_ros` does not do this, so either create your own copy,
  /// or if building `gazebo_ros` from source, edit `gazebo_ros/launch/empty_world.launch`
  /// and modify:
  ///
  /// \code
  /// <!-- start gazebo client -->
  ...
  /// <!-- start gazebo client -->
  /// <group if="$(arg gui)">
  ///   <node name="gazebo_gui" pkg="gazebo_ros" type="gzclient" respawn="false" output="screen" args="$(arg command_arg3) $(arg extra_gazebo_args)"/>
  /// </group>
  /// \endcode
  ///
